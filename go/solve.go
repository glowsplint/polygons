...
func GetPolygonVertex(i int, theta dec.Decimal, precision int32, tolerance dec.Decimal, points chan Point) {
	...
	points <- Point{x.Round(precision), y.Round(precision)}
}

func (ps PolygonSolver) CreatePolygonVerticesConcurrently(precision int32, tolerance dec.Decimal) ByXY {
	// Returns the polygon vertices generated concurrently
	theta := TWO.Div(dec.NewFromInt(int64(ps.n))).Mul(dec.NewFromFloat(math.Pi))
	points := make(chan Point)
	polygon := make([]Point, 0)
	for i := 0; i < ps.n; i++ {
		go GetPolygonVertex(i, theta, precision, tolerance, points)
	}
	for i := 0; i < ps.n; i++ {
		point := <-points
...
func (ps PolygonSolver) DrawLineSegments(polygonVertices ByXY, precision int32, tolerance dec.Decimal) LineSegmentSlice {
...
func main() {
	...
	ps := PolygonSolver{60}
	polygonVertices := ps.CreatePolygonVertices(p, t)
	lineSegments := ps.DrawLineSegments(polygonVertices, p, t)
